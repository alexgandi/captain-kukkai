...
export default class WardrobeScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Griglia 6 x 2 (11 costumi: streak e Collector compresi).
    const cols = 6;
    const cw = 128;
    ...
    COSTUMES.forEach((c, i) => {
      ...
      const draw = (sel) => {
        ...
        bg.fillRoundedRect(-60, -42, 120, 84, 12);
        bg.lineStyle(3, sel ? 0xffd166 : unlocked ? 0x4a6b83 : 0x33465a, 1);
        bg.strokeRoundedRect(-60, -42, 120, 84, 12);
      };
      ...
      const hint = this.add.text(0, 27, unlocked ? '' : c.hint, { fontFamily: 'sans-serif', fontSize: '10px', color: '#8fb0c6', wordWrap: { width: 112 }, align: 'center' }).setOrigin(0.5);
      card.add([bg, emoji, name, hint]);
      card.setSize(120, 84);
      if (unlocked) {
        card.setInteractive(new Phaser.Geom.Rectangle(-60, -42, 120, 84), Phaser.Geom.Rectangle.Contains);
